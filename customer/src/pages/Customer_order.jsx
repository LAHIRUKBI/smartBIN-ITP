...
export default function Customer_order() {
  ...
  const handleDownloadReceipt = (request) => {
    ...
    const tableData = [
      ...
      { title: 'Submitted At', value: new Date(request.submittedAt).toLocaleString() },
    ];
    ...
    doc.setFontSize(12);
    doc.text('Thank you for choosing smartBIN! Your contribution helps us create a cleaner environment.', 14, pageHeight - 50);
    ...
    <div className="min-h-screen bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300 text-gray-900 flex flex-col">
      <div className="flex-grow py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl font-extrabold text-center mb-8 text-teal-600">Your Orders</h1>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredRequests.length > 0 ? (
              filteredRequests.map(request => {
                ...
                return (
                  <div key={request._id} className="bg-gray-100 rounded-lg shadow-lg border border-gray-200 transition-transform transform hover:scale-105 p-6 flex flex-col">
                    <CountdownTimer startTime={endTime.toISOString()} />

                    <div className="bg-white p-4 rounded-lg mb-4 shadow-md">
                      <h2 className="text-2xl font-bold text-teal-600 mb-4">{request.name}</h2>
                      <div className="border-t border-gray-300 pt-4">
                        <p className="text-gray-700 mb-2"><strong>Email:</strong> {request.email}</p>
                        <p className="text-gray-700 mb-2"><strong>Phone:</strong> {request.phone}</p>
                        <p className="text-gray-700 mb-2"><strong>Address:</strong> {request.address}</p>
                        <p className="text-gray-700 mb-2"><strong>Additional Info:</strong> {request.additionalInfo || 'N/A'}</p>
                        <p className="text-gray-700 mb-2"><strong>Service ID:</strong> {request.serviceId}</p>
                        <p className="text-gray-700 mb-2"><strong>Payment Method:</strong> {request.paymentMethod}</p>
                        <p className="text-gray-700 mb-2"><strong>Submitted At:</strong> {new Date(request.submittedAt).toLocaleString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto flex gap-4">
                      ...
                      <button 
                        ...
                        className="flex items-center justify-center bg-red-600 hover:bg-red-500 text-white p-3 rounded transition-colors duration-300 shadow-md hover:shadow-lg w-full"
                      >
                        <FaTrash className="mr-2" /> Delete
                      </button>
                      <button 
                        onClick={() => handleDownloadReceipt(request)} 
                        className="flex items-center justify-center bg-blue-600 hover:bg-blue-500 text-white p-3 rounded transition-colors duration-300 shadow-md hover:shadow-lg w-full"
                      >
                        <FaFilePdf className="text-lg" /> {/* Only icon here */}
                      </button>
                    </div>
                  </div>
                );
              ...
              <div className="p-6 text-center text-gray-400">No service requests found.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <footer className="bg-green-800 text-white text-center py-6">
        <div className="container mx-auto">
          ...
          <div className="flex justify-center space-x-4">
            <button className="bg-green-600 hover:bg-green-500 text-white p-3 rounded">Contact Us</button>
            <button className="bg-green-600 hover:bg-green-500 text-white p-3 rounded">Learn More</button>
          </div>
        </div>
      </footer>
    </div>
